...
# 函数，根据option值和当前菜单，返回内嵌的下一级菜单（字典）
def get_embed_dic(current_dic, option):
    for num, item in enumerate(current_dic):
        if num + 1 == int(option):
            return current_dic[item]
    ...
    while option != 'q':
        ...
        # 如果输入的option存在于当前菜单编号列表中
            ...
            if type(current_product_list[product]) == int :
                ...
                price = get_price(PRODUCT_DIC,product)
                cost = current_product_list[product] * quantity
                # 如果总价大于余额，提示余额不够
                if cost > money:
